...
[newStateFront,tsqFront,inpFront] = frontPart3(init,final,consts2);
[newStateBack,tsqBack,inpBack] = backPart3(init,final,consts2);
[tsq,inp,profileType] = calc_mintime_traj_7(newStateFront{4},newStateBack{1},input);

newInit{1} = newStateFront{4}; newInit{2} = sum(tsqFront);
...
if ~isempty(tsq)    
    ...
    if abs(vel(4:10)) > input(3)
        keyboard
    end
    
    if abs(acc(4:10)) > input(2)
        keyboard
    end
else
%     keyboard
